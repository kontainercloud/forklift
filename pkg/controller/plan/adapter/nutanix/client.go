...
func (r *Client) ensureImage(vmRef ref.Ref, disk model.Disk) (ready bool, err error) {
	...
	entity, err := r.findImageByName(name)
	if err != nil {
		return false, err
	}
	if entity == nil {
...
// findImageByName returns the v3 image entity with the given spec.name. entity
// will be nil if it is not found. Nutanix doesn't guarantee image names are
// unique, but names generated by migrationImageName are, so the first match
// is authoritative.
func (r *Client) findImageByName(name string) (entity *libclient.V3Image, err error) {
	entities, err := libclient.ListAllV3[libclient.V3Image](
		&r.Client,
		"image",
		imagePageSize,
		libclient.V3ImageNameFilter(name),
	)
	if err != nil {
		return nil, err
	}
